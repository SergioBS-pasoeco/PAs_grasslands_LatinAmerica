### SDM key species for conservation - with accuracy metrics and spatial blocks

library(sf)
library(terra)

# setwd("H:/SDM_project/GBIF")
setwd("E:/SDM_project")

#Birds_CO_PA = sf::st_read("Birds/Birds_CO/Birds_COL_PA.shp")
#V_PA = terra::vect("Birds/Birds_CO/Birds_COL_PA.shp")
#terra::writeVector(V_PA, "Birds/Birds_CO/Birds_COL_PA.gpkg", overwrite = TRUE)
# Birds_CO_PA <- terra::vect("Birds/Birds_CO/Birds_COL_PA.gpkg")
Birds_CO_PA <- terra::vect("Vector/Birds/Birds_CO/Birds_COL_PA.gpkg")


#Birds_CO_oPA = sf::st_read("Birds/Birds_CO/Birds_COL_outPA.shp")
#V_oPA = terra::vect("Birds/Birds_CO/Birds_COL_outPA.shp")
#terra::writeVector(V_oPA, "Birds/Birds_CO/Birds_COL_outPA.gpkg", overwrite = TRUE)
# Birds_CO_oPA <- terra::vect("Birds/Birds_CO/Birds_COL_outPA.gpkg")
Birds_CO_oPA <- terra::vect("Vector/Birds/Birds_CO/Birds_COL_outPA.gpkg")

# aggregate by year -------------------------------------------------------
library(tidyverse)

Birds_CO_PA_df <- as.data.frame(terra::values(Birds_CO_PA))
Birds_CO_oPA_df <- as.data.frame(terra::values(Birds_CO_oPA))
dat_all_CO <- bind_rows(Birds_CO_PA_df, Birds_CO_oPA_df)
dat_all_CO$type = c(rep("Reserve",nrow(Birds_CO_PA)), rep( "No Reserve", nrow(Birds_CO_oPA)))
dat_all_CO$country = rep("Colombia",nrow(dat_all_CO))

# Unique species reported inside PAs but not outside
sp_reserves = unique(dat_all_CO[which(dat_all_CO$type== "Reserve"),]$species)
sp_reserves_nr = unique(dat_all_CO[which(dat_all_CO$type== "No Reserve"),]$species)
target_species = setdiff(sp_reserves, sp_reserves_nr)
length(target_species)
length(unique(dat_all_CO$species))

# Species distribution model ----------------------------------------------
# install.packages(c("sf", "terra", "dismo", "geodata", "dplyr", "purrr", "maxnet"))
...
# ndvi_mean_path <- "data/raw/rasters/mean_ndvi_250m_CO.tif"
# ndvi_sd_path <- "data/raw/rasters/std_dev_ndvi_250m_CO.tif"
...
#   ndvi_mean_path <- "data/raw/rasters/mean_ndvi_500m_CO.tif"
#   ndvi_sd_path <- "data/raw/rasters/std_dev_ndvi_500m_CO.tif"
# }
# m_ndvi <- terra::rast(ndvi_mean_path)
# sd_ndvi <- terra::rast(ndvi_sd_path)
ndvi_mean_path <- "Raster/mean_ndvi_250m_CO.tif"
ndvi_sd_path <- "Raster/std_dev_ndvi_250m_CO.tif"
if (!file.exists(ndvi_mean_path) || !file.exists(ndvi_sd_path)) {
  warning("250 m NDVI files not found; falling back to 500 m NDVI files.")
  ndvi_mean_path <- "Raster/mean_ndvi_500m_CO.tif"
  ndvi_sd_path <- "Raster/std_dev_ndvi_500m_CO.tif"
}
m_ndvi <- terra::rast(ndvi_mean_path)
sd_ndvi <- terra::rast(ndvi_sd_path)

# Load terrain rasters at 250m resolution (Colombia/Casanare only)
# terrain_casanare <- terra::rast("data/raw/rasters/terrain_stack_casanare_resample_250m.tif")
terrain_casanare <- terra::rast("Raster/terrain_stack_casanare_resample_250m.tif")

# Load population density raster at 250m resolution
# gpw_casanare <- terra::rast("data/raw/rasters/Resampled_250m_GPW_CO.tif")
gpw_casanare <- terra::rast("Raster/Resampled_250m_GPW_CO.tif")
...
run_sdm_loop <- function(gadm_sf, occurrences_df, target_species, resolution = 2.5, sdm_method = "maxent", k_folds = 5, seed = 2025, k_blocks = 5) {
  ...
  for (i in seq_along(target_species)) {
    ...
    if (sdm_method == "maxent") {
      ...
      if (length(fold_metrics) > 0) {
        ...
        # fold_metrics_path <- file.path("H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/fold_metrics/250m", paste0(gsub("[ /]", "_", sp), "_fold_metrics.csv"))
        fold_metrics_path <- file.path("E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test/fold_metrics/250m", paste0(gsub("[ /]", "_", sp), "_fold_metrics.csv"))
      ...
      if (!is.null(prediction)) {
        # out_path <- file.path("H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/250m", paste0(gsub("[ /]", "_", sp), "_SDM.tif"))
        out_path <- file.path("E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test", paste0(gsub("[ /]", "_", sp), "_SDM.tif"))
  ...
  # fold_metrics_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/fold_metrics"
  fold_metrics_dir <- "E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test/fold_metrics"
...
# Running the loop --------------------------------------------------------
# gadm_Cas = terra::vect("Vectors/CO/Casanare.shp")
gadm_Cas = terra::vect("Vector/Casanare.shp")
...
target_species = target_species_test[193:length(target_species_test)],
  resolution = 2.5,
  sdm_method = "maxent",
  k_folds = 5,
  seed = 2025,
...
# out_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test"
out_dir <- "E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test"
...
if (!is.null(varimp_all_CO)) {
  ...
  out_csv <- file.path(out_dir, "CO_birds_varimp_with_metrics_250.csv")
...
# folder_path <- "H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/fold_metrics/250m"
folder_path <- "E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test/fold_metrics/250m"
...
if (!is.null(fold_metrics_CO)) {
  ...
  # fold_metrics_out <- "H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/combined_fold_metrics_250.csv"
  fold_metrics_out <- "E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test/combined_fold_metrics_250.csv"
  ...
  # Create boxplots
  # boxplot_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/CO_ndvi_terrain_test/boxplots"
  boxplot_dir <- "E:/SDM_project/results/SDM_birds/CO_ndvi_terrain_test/boxplots"
