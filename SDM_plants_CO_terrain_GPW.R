### SDM plants Colombia - with terrain, GPW, accuracy metrics, and spatial blocks
### Based on SDM_plants_CO_new.R structure, extended with terrain/GPW predictors
...
# ---- Inputs ---------------------------------------------------------------
# plants_pa  <- terra::vect("GBIF/Plants/Plants_COL_PA.shp")
plants_pa  <- terra::vect("Vector/Plants/Plants_COL_PA.shp")
# plants_opa <- terra::vect("GBIF/Plants/Plants_COL_outPA.shp")
plants_opa <- terra::vect("Vector/Plants/Plants_COL_outPA.shp")
dat_pa     <- as.data.frame(terra::values(plants_pa))
dat_opa    <- as.data.frame(terra::values(plants_opa))
dat_all_CO <- bind_rows(dat_pa, dat_opa)
dat_all_CO$type    <- c(rep("Reserve", nrow(plants_pa)), rep("No Reserve", nrow(plants_opa)))
dat_all_CO$country <- "Colombia"

# gadm_CO <- terra::vect("Vectors/CO/Casanare.shp")
gadm_CO <- terra::vect("Vector/Casanare.shp")
...
# Terrain stack (Casanare, 250 m): elevation, slope, aspect, hillshade
...
# ---- Output directories ---------------------------------------------------
# out_dir  <- "H:/SDM_project/GBIF/WK_results/SDM_plants/CO_terrain_GPW_250m"
out_dir  <- "E:/SDM_project/results/SDM_plants/CO_terrain_GPW_250m"

# out_dir  <- "C:/Users/santamaria/Documents/Spatial_analysis/GBIF/SDM_project/result_test/SDM_plants/CO_terrain_GPW_250m"
...
results_CO <- run_sdm_loop(
  ...
  target_species  = target_species_test[1:length(target_species_test)],   # adjust range as needed
  resolution      = 2.5,
  k_folds         = 5,
  seed            = 2025,
  k_blocks        = 5
...
fold_metrics_CO <- results_CO$fold_metrics
...
if (!is.null(varimp_all_CO)) {
  ...
  out_csv <- file.path(out_dir, "CO_plants_varimp_with_metrics_250.csv")
