### SDM plants Argentina - with terrain, GPW, accuracy metrics, and spatial blocks
### Based on SDM_plants_AR_new.R structure, extended with terrain/GPW predictors
### and permutation-based variable importance as in SDM_birds_CO_acc_metrics_V3_OK.R
...
# Allow terra to use more RAM and process rasters in chunks rather than
# loading them fully into memory -- prevents GDAL readValues OOM errors
# on large 250 m rasters. Adjust memfrac if your machine has more RAM.
# Redirect terra temp files to a controlled folder on the same drive as outputs.
# This prevents the system temp partition from filling up when processing
# large 250 m rasters across many species.
terra_tmp <- file.path("E:/SDM_project", "terra_tmp")
dir.create(terra_tmp, showWarnings = FALSE, recursive = TRUE)
terra::terraOptions(memfrac = 0.6, progress = 0, tempdir = terra_tmp)

# setwd("H:/SDM_project")
setwd("E:/SDM_project")

# ---- Inputs ---------------------------------------------------------------
# plants_pa  <- terra::vect("GBIF/Plants/Plants_ARG_PA.shp")
plants_pa  <- terra::vect("Vector/Plants/Plants_ARG_PA.shp")

# plants_opa <- terra::vect("GBIF/Plants/Plants_ARG_outPA.shp")
plants_opa <- terra::vect("Vector/Plants/Plants_ARG_outPA.shp")
dat_pa     <- as.data.frame(terra::values(plants_pa))
dat_opa    <- as.data.frame(terra::values(plants_opa))
dat_all_AR <- bind_rows(dat_pa, dat_opa)
dat_all_AR$type    <- c(rep("Reserve", nrow(plants_pa)), rep("No Reserve", nrow(plants_opa)))
dat_all_AR$country <- "Argentina"

# gadm_AR <- terra::vect("Vectors/AR/Buenos_Aires_Province.shp")
gadm_AR <- terra::vect("Vector/Buenos_Aires_Province.shp")

# ---- Load rasters ---------------------------------------------------------

# setwd("C:/Users/santamaria/Documents/Spatial_analysis/GBIF/SDM_project")
# NDVI (250 m) – prefer 250 m, fall back to 500 m if needed
ndvi_mean_path <- "Raster/mean_ndvi_250m_AR.tif"
ndvi_sd_path   <- "Raster/std_dev_ndvi_250m_AR.tif"
if (!file.exists(ndvi_mean_path) || !file.exists(ndvi_sd_path)) {
  ...
  ndvi_mean_path <- "Raster/mean_ndvi_500m_AR.tif"
  ndvi_sd_path   <- "Raster/std_dev_ndvi_500m_AR.tif"
}

# ndvi_mean_path <- "data/raw/rasters/mean_ndvi_250m_AR.tif"
# ndvi_sd_path   <- "data/raw/rasters/std_dev_ndvi_250m_AR.tif"
# if (!file.exists(ndvi_mean_path) || !file.exists(ndvi_sd_path)) {
#   warning("250 m NDVI files not found; falling back to 500 m NDVI files.")
#   ndvi_mean_path <- "data/raw/rasters/mean_ndvi_500m_AR.tif"
#   ndvi_sd_path   <- "data/raw/rasters/std_dev_ndvi_500m_AR.tif"
# }
...
# Terrain stack (Buenos Aires Province, 250 m): elevation, slope, aspect, hillshade

# terrain_bsas <- terra::rast("data/raw/rasters/terrain_stack_BA_resampled_250m.tif")
terrain_bsas <- terra::rast("Raster/terrain_stack_BA_resampled_250m.tif")
names(terrain_bsas) <- c("elevation", "slope", "aspect", "hillshade")

# Population density – GPW (250 m)

# gpw_bsas <- terra::rast("data/raw/rasters/Resampled_250m_GPW_ARG.tif")
gpw_bsas <- terra::rast("Raster/Resampled_250m_GPW_ARG.tif")
names(gpw_bsas) <- "gpw_density"

# ---- Output directories ---------------------------------------------------
# out_dir  <- "H:/SDM_project/GBIF/WK_results/SDM_plants/AR_terrain_GPW_250m"
out_dir  <- "E:/SDM_project/results/SDM_plants/AR_terrain_GPW_250m"
...
pick_coord_cols <- function(df) {
  lon_candidates <- c("decimalLon", "decimalLongitude", "dcmlLng", "longitude", "lon")
  lat_candidates <- c("decimalLat", "decimalLatitude", "dcmlLtt", "latitude", "lat")
...
# ---- Boxplot helper (mirrors birds script) --------------------------------
...
run_sdm_loop <- function(gadm_sf, occurrences_df, target_species,
  ...
  for (i in seq_along(target_species)) {
    ...
    # 5. Terrain (250 m, Buenos Aires Province)
    terrain_stack    <- terrain_bsas
    ...
    # Use terra::global() instead of terra::values() -- reads block-by-block,
    # never loads the full raster into RAM (avoids GDAL readValues OOM errors)
    valid_layers <- sapply(seq_len(nlyr(env_with_ndvi)), function(k) {
      n_finite <- tryCatch(
        terra::global(env_with_ndvi[[k]], fun = "notNA")[[1]],
        error = function(e) 0L
      )
      n_finite > 0
    })
    ...
    # ---- Spatial block CV -------------------------------------------------
    # Assign block labels to the already-cleaned sdm_data (presences + capped
    # background).  Splitting this single unified dataset guarantees that every
    # test fold can contain both presences and background, which was impossible
    # in the previous approach that built env_fold from only the occurrences
    # inside block b and all background points indiscriminately.
    blocks_sdm <- make_spatial_blocks(coords_df[, c("lon", "lat")],
                                      k_blocks = k_blocks, seed = seed)
    ...
    for (b in sort(unique(blocks_sdm))) {
      
      test_ids  <- which(blocks_sdm == b)
      train_ids <- setdiff(seq_len(nrow(sdm_data)), test_ids)
      
      if (length(test_ids) == 0 || length(train_ids) < 10) next
      
      pres_train <- sdm_data$presence[train_ids]
      pres_test  <- sdm_data$presence[test_ids]
      
      if (anyNA(pres_train) || anyNA(pres_test)) next
      
      # Train: need >= 8 presences and at least some background
      if (sum(pres_train == 1L) < 8 || sum(pres_train == 0L) < 1) {
        message("  Block ", b, " skipped for ", sp,
                ": train has pres=", sum(pres_train == 1L),
                " bg=", sum(pres_train == 0L))
        next
      }
      # Test: need at least 1 presence AND 1 background for evaluation
      if (sum(pres_test == 1L) < 1 || sum(pres_test == 0L) < 1) {
        message("  Block ", b, " skipped for ", sp,
                ": test has pres=", sum(pres_test == 1L),
                " bg=", sum(pres_test == 0L))
        next
      }
      ...
      mdl_b <- tryCatch({
        maxnet::maxnet(
          p    = pres_train,
          data = sdm_data[train_ids, feature_names, drop = FALSE],
        ...
        message("  Error building model for ", sp, " block ", b, ": ", e$message)
        NULL
      })
      if (is.null(mdl_b)) next
      
      X_test <- sdm_data[test_ids, feature_names, drop = FALSE]
      if (nrow(X_test) == 0 || ncol(X_test) == 0) next
      
      preds <- tryCatch(
        stats::predict(mdl_b, newdata = X_test, type = "cloglog"),
        error = function(e) {
          message("  Error predicting for ", sp, " block ", b, ": ", e$message)
          rep(NA_real_, nrow(X_test))
        }
      )
      
      pred_all   <- c(pred_all,   preds)
      labels_all <- c(labels_all, pres_test)
      
      valid_fold  <- is.finite(preds)
      labels_fold <- as.integer(pres_test[valid_fold])
      scores_fold <- preds[valid_fold]
      
      # Final check after filtering NAs
      if (sum(labels_fold == 1L) < 1 || sum(labels_fold == 0L) < 1) next
      
      fold_counter  <- fold_counter + 1
      n_pres_fold   <- sum(labels_fold == 1L)
      n_bg_fold     <- sum(labels_fold == 0L)
      baseline_fold <- n_pres_fold / (n_pres_fold + n_bg_fold)
      
      roc_auc_fold <- tryCatch(
        dismo::evaluate(p = scores_fold[labels_fold == 1L],
                        a = scores_fold[labels_fold == 0L])@auc,
        error = function(e) NA_real_
      )
      auprc_fold <- tryCatch(
        compute_auprc(labels_fold, scores_fold),
        error = function(e) NA_real_
      )
      ...
      tss_thr_fold    <- tss_res_fold$threshold
      auprc_norm_fold <- if (is.finite(auprc_fold) && baseline_fold < 1)
        (auprc_fold - baseline_fold) / (1 - baseline_fold) else NA_real_
      
      fold_metrics <- append(fold_metrics, list(data.frame(
        species    = sp,
        block      = b,
        fold       = fold_counter,
      ...
      message("  Block ", b, " OK: AUC=", round(roc_auc_fold, 3),
              " TSS=", round(tss_fold, 3),
              " (pres=", n_pres_fold, ", bg=", n_bg_fold, ")")
    ...
    message("  Fold metrics collected for ", sp, ": ", fold_counter, " valid block(s)")
    if (length(fold_metrics) > 0) {
      ...
      message("  Saved fold metrics: ", fold_metrics_path,
              " (", nrow(fold_metrics_df), " rows, cols: ",
              paste(names(fold_metrics_df), collapse = ", "), ")")
    } else {
      message("  WARNING: no fold metrics saved for ", sp,
              " -- all ", k_blocks, " blocks were skipped (too few presences or bg per block)")
    }
    
    # ---- Robust TSS threshold: median across blocks ----------------------
    # The pooled tss_thr (computed above on all out-of-block predictions) can
    # be pulled to extreme values by a single dominant block with poor
    # discrimination.  The median of per-block thresholds is more stable.
    if (length(fold_metrics) > 0) {
      fold_metrics_df  <- do.call(rbind, fold_metrics)
      tss_thr_robust   <- stats::median(fold_metrics_df$tss_thr, na.rm = TRUE)
      message("  Robust TSS threshold (median of ", nrow(fold_metrics_df),
              " blocks): ", round(tss_thr_robust, 4),
              "  [pooled was: ", round(tss_thr, 4), "]")
    } else {
      tss_thr_robust <- tss_thr   # fall back to pooled if no block folds succeeded
    }
    ...
    if (!is.null(model)) {
      ...
      # Only rename layers when counts match; mismatches silently break terra::predict
      if (nlyr(env_for_pred) == length(vars)) {
        names(env_for_pred) <- vars
      } else {
        message("  WARNING: layer count (", nlyr(env_for_pred),
                ") != predictor count (", length(vars),
                ") for ", sp, " -- keeping original layer names")
      }
      prediction <- tryCatch(
      ...
      if (!is.null(prediction)) {
        ...
        if (is.finite(tss_thr_robust)) {
          binary_map <- prediction >= tss_thr_robust
          plot(binary_map, main = paste0(i, "_", gsub(" ", "_", sp),
                                         " - Binary (median TSS thr=", round(tss_thr_robust, 3), ")"))
          plot(vect(study_area), add = TRUE, border = "white", lwd = 1.2)
          plot(bg_points_main, add = TRUE, pch = 20, cex = 0.4, col = "white")
          plot(vect(occ_sf), add = TRUE, pch = 19, cex = 0.6, col = "red")
        } else {
          # Last resort: spatSample-based median (avoids GDAL OOM)
          med_thr <- tryCatch({
            samp <- terra::spatSample(prediction, size = 10000,
                                      method = "random", na.rm = TRUE,
                                      as.raster = FALSE)
            stats::median(samp[[1]], na.rm = TRUE)
          }, error = function(e) NA_real_)
          if (is.finite(med_thr)) {
            binary_map <- prediction >= med_thr
            plot(binary_map, main = paste0(i, "_", gsub(" ", "_", sp),
                                           " - Binary (raster median thr=", round(med_thr, 3), ")"))
          } else {
            plot(prediction, main = paste0(i, "_", gsub(" ", "_", sp),
                                           " - Binary (threshold unavailable)"))
          }
          plot(vect(study_area), add = TRUE, border = "white", lwd = 1.2)
          plot(bg_points_main, add = TRUE, pch = 20, cex = 0.4, col = "white")
          plot(vect(occ_sf), add = TRUE, pch = 19, cex = 0.6, col = "red")
      ...
      # ------------------------------------------------------------------ #
      #  Per-species RAM cleanup                                            #
      #  All exports (TIF + fold CSV) are done above. Remove every         #
      #  species-specific object to free RAM before the next iteration.    #
      #  Preserved: m_ndvi, sd_ndvi, terrain_casanare, gpw_casanare        #
      #             (base rasters, reused every species),                  #
      #             gadm_sf / gadm_Cas (study boundary),                   #
      #             varimp_rows, processed_count, skipped_count            #
      #             (cross-species accumulators).                          #
      # ------------------------------------------------------------------ #
      
      # 1. Species occurrence objects
      rm(list = intersect(ls(), c(
        "sp_occ", "sp_sf", "occ_sf", "occ_coords", "occ_points"
      )))
      
      # 2. Raster intermediates (crop/mask/resample products) and combined stack
      rm(list = intersect(ls(), c(
        "env", "study_env", "env_crop", "env_masked", "env_resampled",
        "ndvi_stack", "study_ndvi", "ndvi_crop", "ndvi_masked",
        "terrain_stack", "study_terrain", "terrain_crop",
        "terrain_masked", "terrain_resampled",
        "gpw_stack", "study_gpw", "gpw_crop", "gpw_masked", "gpw_resampled",
        "env_with_ndvi", "env_for_pred"
      )))
      
      # 3. Extracted environmental tables and model data frames
      rm(list = intersect(ls(), c(
        "env_all", "env_vals_occ", "env_vals_bg_pool", "env_vals_bg",
        "env_fold", "sdm_data", "sdm_fold",
        "coords_df", "coords_fold", "coords_all_pool",
        "presence_vec", "presence_fold", "keep", "keep_f", "keep_rows",
        "pres", "abs", "pres_idx", "abs_idx", "keep_abs",
        "predictor_names", "feature_names", "vars",
        "valid_layers", "finite_ok", "zero_var"
      )))
      
      # 4. Background point objects
      rm(list = intersect(ls(), c(
        "bg_area", "bg_points_pool", "bg_pool_coords", "bg_pool_sf",
        "bg_coords", "bg_points_main",
        "buffer_5km", "buffer_union", "buffer_union_ll",
        "buffer_vect", "study_vect", "pd_aligned", "pd_in_study",
        "sp_proj"
      )))
      
      # 5. Spatial block objects
      rm(list = intersect(ls(), c(
        "blocks_all", "blocks_fold", "pres_block_ids",
        "coords_fold", "rows_all"
      )))
      
      # 6. Fitted models and fold-level objects
      rm(list = intersect(ls(), c(
        "model", "mdl_b", "mdl_f", "X_test", "Xp",
        "preds", "pred_all", "labels_all",
        "base_pred", "base_pred_k", "perm_pred", "perm_preds",
        "folds", "folds_info", "fold_counter",
        "labels", "scores", "labels_fold", "scores_fold",
        "valid", "valid_fold"
      )))
      
      # 7. Metrics and variable importance intermediates (already in varimp_rows)
      rm(list = intersect(ls(), c(
        "roc_auc", "auprc", "auprc_norm", "tss", "tss_thr", "tss_res",
        "baseline", "denom", "n_pres", "n_abs", "n_pres_eval", "n_bg_eval",
        "base_auc", "drops", "total",
        "roc_auc_fold", "auprc_fold", "auprc_norm_fold", "tss_fold",
        "tss_res_fold", "baseline_fold", "n_pres_fold", "n_bg_fold",
        "denom_fold", "fold_metrics", "fold_metrics_df",
        "folds", "k_eff", "n_all", "f", "b"
      )))
      
      # 8. Raster prediction outputs (already written to disk)
      rm(list = intersect(ls(), c(
        "prediction", "binary_map", "out_path"
      )))
      
      # 9. Study area helper objects (recreated each iteration from gadm_sf)
      rm(list = intersect(ls(), c(
        "study_area", "bbox"
      )))
      
      # Force garbage collection to return freed memory to the OS
      gc(verbose = FALSE)
      
      message("  RAM freed after species: ", sp)
...
results_AR <- run_sdm_loop(
  gadm_sf         = gadm_AR,
  occurrences_df  = dat_all_AR,
  target_species  = target_species_test[540:length(target_species_test)],   # adjust range as needed
  resolution      = 2.5,
  k_folds         = 5,
  seed            = 2025,
  k_blocks        = 5
...
# ---- Post-processing ------------------------------------------------------
varimp_all_AR   <- results_AR$varimp
fold_metrics_AR <- results_AR$fold_metrics

message("Total species processed: ", ifelse(is.null(varimp_all_AR), 0, length(unique(varimp_all_AR$species))))

# Variable importance + metrics wide table
if (!is.null(varimp_all_AR)) {
  wide_AR <- tidyr::pivot_wider(varimp_all_AR,
                                id_cols     = species,
                                names_from  = variable,
                                values_from = perc_contribution,
                                values_fill = 0)
  metrics_df <- varimp_all_AR %>%
  ...
  wide_AR <- dplyr::left_join(wide_AR, metrics_df, by = "species")
  ...
  for (old in names(rename_map)) {
    if (old %in% names(wide_AR)) names(wide_AR)[names(wide_AR) == old] <- rename_map[[old]]
  }
  
  out_csv <- file.path(out_dir, "AR_plants_varimp_with_metrics_250.csv")
  dir.create(dirname(out_csv), showWarnings = FALSE, recursive = TRUE)
  utils::write.csv(wide_AR, out_csv, row.names = FALSE)
  message("Saved variable importance + metrics: ", out_csv)
}

# Fold metrics: save combined CSV and produce boxplots
if (!is.null(fold_metrics_AR)) {
  message("Fold metrics available for ", length(unique(fold_metrics_AR$species)), " species")
  
  fold_metrics_out <- file.path(out_dir, "combined_fold_metrics_250.csv")
  utils::write.csv(fold_metrics_AR, fold_metrics_out, row.names = FALSE)
  message("Saved combined fold metrics to: ", fold_metrics_out)
  
  boxplot_dir <- file.path(out_dir, "boxplots")
  create_metric_boxplots(fold_metrics_AR, boxplot_dir)
} else {
