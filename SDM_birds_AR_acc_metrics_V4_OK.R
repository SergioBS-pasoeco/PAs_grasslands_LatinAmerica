# SDM key species for conservation - Argentina (AR) with accuracy metrics
...
# ---- Inputs ---------------------------------------------------------------
# V_PA = terra::vect("GBIF/Birds/Birds_AR/Birds_ARG_PA.shp")
# terra::writeVector(V_PA, "GBIF/Birds/Birds_AR/Birds_ARG_PA.gpkg", overwrite = TRUE)
# birds_pa <- terra::vect("GBIF/Birds/Birds_AR/Birds_ARG_PA.gpkg")
birds_pa <- terra::vect("Vector/Birds/Birds_AR/Birds_ARG_PA.gpkg")

# V_oPA = terra::vect("GBIF/Birds/Birds_AR/Birds_ARG_outPA.shp")
# terra::writeVector(V_oPA, "GBIF/Birds/Birds_AR/Birds_ARG_outPA.gpkg", overwrite = TRUE)
# birds_opa <- terra::vect("GBIF/Birds/Birds_AR/Birds_ARG_outPA.gpkg")
birds_opa <- terra::vect("Vector/Birds/Birds_AR/Birds_ARG_outPA.gpkg")

birds_pa_df <- as.data.frame(terra::values(birds_pa))
birds_opa_df <- as.data.frame(terra::values(birds_opa))

dat_all_AR <- dplyr::bind_rows(birds_pa_df, birds_opa_df)
dat_all_AR$type <- c(rep("Reserve", nrow(birds_pa)), rep("No Reserve", nrow(birds_opa)))
dat_all_AR$country <- "Argentina"

sp_reserves <- unique(dat_all_AR[dat_all_AR$type == "Reserve", ]$species)
sp_reserves_nr <- unique(dat_all_AR[dat_all_AR$type == "No Reserve", ]$species)
target_species <- setdiff(sp_reserves, sp_reserves_nr)

# gadm_AR <- terra::vect("Vectors/AR/Shape_reservas_FVS_Ana/Pampa_deprimida.shp")
# m_ndvi <- terra::rast("data/raw/rasters/mean_ndvi_250m_AR.tif")
# sd_ndvi <- terra::rast("data/raw/rasters/std_dev_ndvi_250m_AR.tif")
# terrain_ar <- terra::rast("data/raw/rasters/terrain_stack_BA_resampled_250m.tif")
# gpw_ar <- terra::rast("data/raw/rasters/Resampled_250m_GPW_ARG.tif")
gadm_AR <- terra::vect("Vector/Buenos_Aires_Province.shp")
m_ndvi <- terra::rast("Raster/mean_ndvi_250m_AR.tif")
sd_ndvi <- terra::rast("Raster/std_dev_ndvi_250m_AR.tif")
terrain_ar <- terra::rast("Raster/terrain_stack_BA_resampled_250m.tif")
gpw_ar <- terra::rast("Raster/Resampled_250m_GPW_ARG.tif")

names(m_ndvi) <- "ndvi_mean"
names(sd_ndvi) <- "ndvi_sd"
names(terrain_ar) <- c("elevation", "slope", "aspect", "hillshade")
names(gpw_ar) <- "gpw_density"

# out_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/250m"
out_dir <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/250m"
...
# terrain_casanare <- terra::rast("data/raw/rasters/terrain_stack_BA_resampled_250m.tif")
terrain_casanare <- terra::rast("Raster/terrain_stack_BA_resampled_250m.tif")

# Load population density raster at 250m resolution
# gpw_casanare <- terra::rast("data/raw/rasters/Resampled_250m_GPW_ARG.tif")
gpw_casanare <- terra::rast("Raster/Resampled_250m_GPW_ARG.tif")
...
run_sdm_loop <- function(gadm_sf, occurrences_df, target_species, resolution = 2.5, sdm_method = "maxent", k_folds = 5, seed = 2025, k_blocks = 5) {
  ...
  for (i in seq_along(target_species)) {
    ...
    if (sdm_method == "maxent") {
      ...
      if (length(fold_metrics) > 0) {
        ...
        # fold_metrics_path <- file.path("H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/fold_metrics/250m", paste0(gsub("[ /]", "_", sp), "_fold_metrics.csv"))
        fold_metrics_path <- file.path("E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/fold_metrics/250m", paste0(gsub("[ /]", "_", sp), "_fold_metrics.csv"))
      ...
      if (!is.null(prediction)) {
        # out_path <- file.path("H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/250m", paste0(gsub("[ /]", "_", sp), "_SDM.tif"))
        out_path <- file.path("E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test", paste0(gsub("[ /]", "_", sp), "_SDM.tif"))
  ...
  # fold_metrics_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/fold_metrics"
  fold_metrics_dir <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/fold_metrics"
...
# Running the loop --------------------------------------------------------
# gadm_Cas = terra::vect("Vectors/AR/Buenos_Aires_Province.shp")
gadm_Cas = terra::vect("Vector/Buenos_Aires_Province.shp")
...
# length(target_species)
  ...
  results_CO <- run_sdm_loop(
    ...
    target_species = target_species_test[14:length(target_species_test)],
    resolution = 2.5,
    sdm_method = "maxent",
    k_folds = 5,
    seed = 2025,
...
# out_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test"
out_dir <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test"
...
if (!is.null(varimp_all_CO)) {
  ...
  out_csv <- file.path(out_dir, "AR_birds_varimp_with_metrics_250.csv")
...
# folder_path <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/fold_metrics/250m"
folder_path <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/fold_metrics/250m"
...
if (!is.null(fold_metrics_CO)) {
  ...
  # fold_metrics_out <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/combined_fold_metrics_250.csv"
  fold_metrics_out <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/combined_fold_metrics_250.csv"
  ...
  # Create boxplots
  # boxplot_dir <- "H:/SDM_project/GBIF/WK_results/SDM_birds/AR_ndvi_terrain_test/boxplots"
  boxplot_dir <- "E:/SDM_project/results/SDM_birds/AR_ndvi_terrain_test/boxplots"
